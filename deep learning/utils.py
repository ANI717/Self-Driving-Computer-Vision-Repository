...
def save_checkpoint(model, optimizer, epoch):
    ...
    torch.save(checkpoint, os.path.join(config.CHECKPOINT, f'epoch_{epoch+1}'))
...
def calculate_accuracy(loader, model):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, servo, motor) in enumerate(loop):
            ...
            num_correct += sum(abs(targets - predictions) <= 2).item()
...
# Write Log
def write_log(epoch, accuracy, loss):
    data = np.array([range(1, epoch+2), loss, accuracy]).T
    pd.DataFrame(data, columns=["epoch", 'loss', 'accuracy']).to_csv(os.path.join(config.OUTPUT, 'log.csv'), index=False)